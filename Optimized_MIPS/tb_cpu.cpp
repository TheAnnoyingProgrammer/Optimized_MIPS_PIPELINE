...
static const int W_CLK = 6;
static const int W_VAL = 3;

const std::string HSEP(175, '=');
const std::string SEP(175, '-');
...
int main(int argc, char** argv) {
    ...
    std::cout << "        | " << fw("Program Counter", INSTR_COL)
              << " | Rs | RD1 | RD1 |";
    for (int col = 0; col < per_row; col++) {
        int i = REG_LO + col * 3;
        if (i <= REG_HI) std::cout << " R" << zpad(i, 2);
    }
    std::cout << " |\n";

    std::cout << "    CLK | " << fw("HexInstr", INSTR_COL)
              << " | Rt | RD2 | RD2 |";
    for (int col = 0; col < per_row; col++) {
        int i = REG_LO + col * 3 + 1;
        if (i <= REG_HI) std::cout << " R" << zpad(i, 2);
    }
    std::cout << " | WAIT A1Bsy A2Bsy\n";

    std::cout << "        | " << fw("Assembly", INSTR_COL)
              << " | Rd | ALU | ALU |";
    for (int col = 0; col < per_row; col++) {
        int i = REG_LO + col * 3 + 2;
        if (i <= REG_HI) std::cout << " R" << zpad(i, 2);
    }
    std::cout << " |\n";
    ...
    for (int cycle = 0; cycle < MAX_CYCLES; cycle++) {
        ...
        std::cout << "        | " << npc_s
                  << " | " << zpad(top->dbg_rs, 2)
                  << " | " << zpad(top->dbg_RD1, W_VAL)
                  << " | " << zpad(top->dbg_RD1, W_VAL)
                  << " |" << reg_row(0) << " |\n";

        std::cout << " " << std::setw(W_CLK) << std::setfill(' ') << cycle
                  << " | " << hexinstr
                  << " | " << zpad(top->dbg_rt, 2)
                  << " | " << zpad(top->dbg_RD2, W_VAL)
                  << " | " << zpad(top->dbg_RD2, W_VAL)
                  << " |" << reg_row(1)
                  << " |  " << (int)top->dbg_WAIT
                  << "     " << (int)top->dbg_ALU1_WAIT
                  << "     " << (int)top->dbg_ALU2_WAIT
                  << "\n";

        std::cout << "        | " << assembly
                  << " | " << zpad(top->dbg_rd, 2)
                  << " | " << zpad(top->dbg_ALU1_Out, W_VAL)
                  << " | " << zpad(top->dbg_ALU2_Out, W_VAL)
                  << " |" << reg_row(2) << " |\n";
